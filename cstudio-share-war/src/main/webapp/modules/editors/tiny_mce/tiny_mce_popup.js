...
(function () {
    ...
    try {
        // Attempt to read the parent window and the tinyMCE namespace
        var win = (!window.frameElement && window.dialogArguments) || opener || parent || top;
        var a = a.tinymce, b = a.tinyMCE;
    } catch (ex) {

        // If access was denied, need to set the document.domain

        var siteName = "";
        var name = "crafterSite=";
        var ca = document.cookie.split(';');

        // Need the site name to fetch the site configuration
        // Find the 'crafterSite' cookie
        for (var c, i = 0, l = ca.length; i < l; i++) {
            c = ca[i];
            while (c.charAt(0) == ' ') {
                c = c.substring(1);
            }
            if (c.indexOf(name) !== -1) {
                siteName = c.substring(name.length, c.length);
                break;
            }
        }
        ...
        if (siteName === '') {
            ...
            console && console.log('tiny_mce_pop.js: crafterSite cookie not found');
        } else {
            ...
            try {
                ...
                xhr.open("GET", "/share/proxy/alfresco/cstudio/site/get-site?key=" + siteName, false);
                xhr.send(null);

                if (xhr.status === 200) {
                    siteConfig = eval('(' + xhr.responseText + ')').site;
                    document.domain = siteConfig.cookieDomain;
                } else {
                    throw 'tiny_mce_pop.js: Getting site configuration returned a unexpected status code (' + xhr.status + ')';
                }
